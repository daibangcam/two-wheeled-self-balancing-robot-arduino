...
void setup() {
 // pinMode(4,OUTPUT);
  //digitalWrite(4 ,HIGH);
  Serial.begin(115200);
  Serial1.begin(9600);//Serial connect to blutooth send data to pc and plot graph
  /*
...
//    }
  ...
  if ((micros() - timerloop) > 6000) {                	//Set time loop update and control motor
    ...
    psi = (mpudata + 1.2) * ToRad;                    	//Read psi value and convert to Rad
    ...
    motorcontrol(PWML, PWMR, (mpudata + 0.0), falldown);
    ...
    String S = "";
    S = (String)(psi * ToDeg) + ',' + (String)(theta * ToDeg) + ',' + (String)(phi * ToDeg) + ',' + (String)(-addtheta * ToDeg) + ',' + (String)(-addphi * ToDeg) + ',' + (String)(leftencoder);
    Serial.println(S);
  }
}
...
float getphi(long lencoder, long rencoder) {    //deg value
  float angle = (9.8 / 29.8) * (lencoder - rencoder);
  return angle;
}
//LQR function//
